#!/usr/bin/env python3
"""
build_careers.py — turn the careers/ folder into the Global Career Links page.

    cd tools && python build_careers.py

One Markdown file per group. Write a list of companies, run this, and they are
on the site. Nothing else to edit.
    ...
    - [Microsoft](https://careers.microsoft.com/) — Azure, Windows, Office, GitHub.
...
Each list item is `- [Company](url)` with an optional note after an em dash or
a hyphen. Anything that is not a link line is ignored, so notes can live in
the file next to the list.

A company may only appear once across the whole folder. The second copy is
dropped with a warning printed here, so the page can never show the same
employer twice however many lists it is merged from.
"""
import json
import os
import re
...
SRC = os.path.join(REPO, "careers")
OUT = os.path.join(REPO, "src", "data", "careers.json")
...
def key(name: str) -> str:
    """Compare company names loosely: 'HP Inc.' and 'HP  inc' are one company."""
...
def main():
    ...
    for name in sorted(os.listdir(SRC)):
        ...
        for link in links:
            k = key(link["name"])
            if k in seen:
                print(f"  duplicate: {link['name']:28} already in {seen[k]}", file=sys.stderr)
                dropped += 1
                continue
            seen[k] = name
            unique.append(link)
        ...
        groups.append(
            {
                ...
                "file": f"careers/{name}",
                "links": sorted(unique, key=lambda x: x["name"].lower()),
            }
        )
