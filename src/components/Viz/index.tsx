...
/* ---------------- tokens: NLP tokenisation ---------------- */

const TOKEN_WIDTHS = [46, 30, 62, 24, 54, 38, 70, 28, 44, 58];

function Tokens(): ReactNode {
  let x = 30;
  const rows: { x: number; y: number; w: number; i: number }[] = [];
  let y = 54;
  TOKEN_WIDTHS.forEach((w, i) => {
    if (x + w > 370) {
      x = 30;
      y += 34;
    }
  ...
  return (
    <svg viewBox={VB} className={styles.svg}>
      {rows.map(({ x: tx, y: ty, w, i }) => (
        <g key={i}>
          ...
          <rect
            ...
            style={{ animationDelay: `${i * 0.16 + 0.25}s` }}
          />
        </g>
      ))}
      <line x1={24} y1={182} x2={376} y2={182} className={styles.axisLine} />
      ...
      <text x={24} y={172} className={styles.axis}>tokens</text>
      <text x={376} y={172} className={styles.axis} textAnchor="end">embeddings</text>
    </svg>
  );
}
...
function Attention(): ReactNode {
  const cells = [];
  for (let r = 0; r < ATT_N; r++) {
    for (let c = 0; c < ATT_N; c++) {
      // causal mask — the upper triangle stays dark, like real decoding
      if (c > r) continue;
      const strength = 0.25 + ((r - c) % 3) * 0.25;
      cells.push({ r, c, strength });
    }
  }
  return (
    <svg viewBox={VB} className={styles.svg}>
      {cells.map(({ r, c, strength }, i) => (
        <rect
          key={i}
          x={110 + c * 26}
          y={40 + r * 26}
          ...
          style={{ animationDelay: `${(r + c) * 0.1}s`, opacity: strength }}
        />
      ))}
      {Array.from({ length: ATT_N }).map((_, i) => (
        <rect
          key={`q${i}`}
          x={70}
          y={40 + i * 26}
      ...
      {Array.from({ length: ATT_N }).map((_, i) => (
        <rect
          ...
          height={10}
          rx={3}
          className={styles.attK}
          style={{ animationDelay: `${i * 0.14 + 0.2}s` }}
        />
      ))}
      <text x={70} y={30} className={styles.axis}>query</text>
      <text x={300} y={30} className={styles.axis} textAnchor="end">key · causal mask</text>
    </svg>
  );
}

/* ---------------- research: hypothesis to paper ---------------- */

const STEPS = ['read', 'hypothesis', 'experiment', 'result', 'paper'];

function Research(): ReactNode {
  return (
    <svg viewBox={VB} className={styles.svg}>
      <line x1={40} y1={96} x2={360} y2={96} className={styles.wire} />
      {STEPS.map((label, i) => (
        <g key={label}>
          <circle
            cx={48 + i * 76}
          ...
          />
          <text x={48 + i * 76} y={136} className={styles.label} textAnchor="middle">
            {label}
          </text>
        </g>
      ))}
      <circle cx={48} cy={96} r={6} className={styles.runner} />

      {/* a results plot being drawn underneath */}
      <rect x={40} y={158} width={320} height={62} rx={8} className={styles.slot} />
